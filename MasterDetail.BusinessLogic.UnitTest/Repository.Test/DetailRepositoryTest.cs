...
namespace MasterDetail.BusinessLogic.UnitTest.Repository.Test
{
    ...
    // as true unit tests here. Only DetailRepository's own logic - building the filter clause and
    ...
    public class DetailRepositoryTests
    {
        ...
        [Fact]
        public async Task GetByIdAsync_ShouldBuildDetailIdFilter_AndDelegateToGetByUniqueKey()
        {
            // Arrange
            var detailId = 1;
        ...
        public async Task GetAllByMasterIdAsync_ShouldBuildMasterIdFilter_AndDelegateToGetByForeignKey()
        {
            // Arrange
            var masterId = 1;
            var expected = new List<DetailRecord> { new DetailRecord { DetailId = 1 } };
            ...
            Assert.Equal(expected[0].DetailId, result.Single().DetailId);
        }
    }
}
